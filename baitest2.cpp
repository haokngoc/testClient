...
int main() {
    ...
    CMD_PACKET packetWork;
    initializePacket(packetWork, 0xAA, 0x58, 0x2b, 0x55, 0x55);
    ...
    // gui ban tin den server lan 1
    ssize_t n_gui1 = sendPacketToServer(client_socket, packetWork);
    // Nhan ban tin 21mb tu server
    const char *output_file1 = "output1.bin";
    receiveAndProcessData(client_socket, output_file1);

    // gui ban tin den server lan 2
    ssize_t n_gui2 = sendPacketToServer(client_socket, packetWork);
    // Nhan ban tin 21mb tu server
    const char *output_file2 = "output2.bin";
    receiveAndProcessData(client_socket, output_file1);

    // gui ban tin den server lan 3
    ssize_t n_gui3 = sendPacketToServer(client_socket, packetWork);
    // Nhan ban tin 21mb tu server
    const char *output_file3 = "output3.bin";
    receiveAndProcessData(client_socket, output_file1);

    // gui ban tin den server lan 4
    ssize_t n_gui4 = sendPacketToServer(client_socket, packetWork);
    // Nhan ban tin 21mb tu server
    const char *output_file4 = "output4.bin";
    receiveAndProcessData(client_socket, output_file1);

    // gui ban tin den server lan 5
    ssize_t n_gui5 = sendPacketToServer(client_socket, packetWork);
    // Nhan ban tin 21mb tu server
    const char *output_file5 = "output5.bin";
    receiveAndProcessData(client_socket, output_file1);
